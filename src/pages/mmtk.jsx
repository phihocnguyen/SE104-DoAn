...
const Mmtk = () => {
      ...
      (async () => {
        console.log(data)
          try {
            ...
            response = await getAllSemesters()
            if (response.status === 200) {
  ...
  const items2 = [
    { label: 'HỌC KỲ', haveDDM: true, position: 'col-span-2', margin: 'justify-center', marrow: 'mr-4', mlabel: 'ml-6', options: ['Chọn học kỳ', 'Học kỳ 1', 'Học kỳ 2', 'Học kì hè'], state: semester, setState: setSemester },
  ...
  return (
    <DefaultLayout value3={item} value={items} value1={items1} value2={items2} label={Label} value4={semesterList} setList={setSemesterList} data={data} setData={setData} >
    </DefaultLayout>
  )
}
export default Mmtk